...
const AssetsManage = () => {
  ...
  const handleAssetStatus = (assetId, status) => {
    ...
    axios({
      ...
      setAssets(prevState => prevState?.map(p => {
        if (p?._id == assetId) {
                  ...
                  <div className="table-responsive">
                    <table className="table">
                      ...
                      <tbody className="table_body">
                        {!loading && assets && assets?.map((asset, index) => {
                          return (
                            <tr key={asset._id}>
                              <td data-label="Date">{new Date(asset?.createdAt)?.toLocaleDateString()}</td>
                                ...
                                {hasWritePermission() ? (
                                  <FormSelect style={{ width: "80%", fontSize: 10, margin: 1 }} onChange={(t) => {
                                    ...
                                    if (currentValue == "Pending") {
                                      ...
                                      handleAssetStatus(asset?._id, currentValue);
                                    }
                                  }}>
                                    <option value="Pending" selected={asset?.status == "Pending"}><small>Pending</small></option>
                                    <option value="Approve" selected={asset?.status == "Approve"}><small>Approve</small></option>
                                    <option value="Reject" selected={asset?.status == "Reject"}><small>Reject</small></option>
                                  </FormSelect>) : showAssetsStatus()}
                              </td>
                              <td data-label="Uploaded By">{asset?.uploadedBy?.email}</td>
