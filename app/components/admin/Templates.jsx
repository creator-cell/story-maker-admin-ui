...
export default function Template() {
  ...
  const getTemplates = async (page = 1, searchTerm = "") => {
    ...
    try {
      ...
      const totalPagesFromApi = res.data.pagination?.totalPages || 1;
      const currentPageFromApi = res.data.pagination?.currentPage - 1 || 0;
      ...
      if (isSuperAdmin) {
        // If a Super admin, show all records without any filtering
        filteredTemplates = allTemplates;
      } else {
        // Otherwise, apply the existing filtering logic
        filteredTemplates = allTemplates.filter((tpl) => {
          // If template has a user, check if it matches the current user
          if (tpl.user) {
            return tpl.user._id === currentUser?._id;
          }
          // If template has no user, only show if its status is 'approved'
          else {
  ...
  const handlePageClick = (selectedItem) => {
    getTemplates(selectedItem.selected + 1, search);
  };
